...
import { Save, Plus, X, Trash2, Lock, CheckCircle, BookOpen } from 'lucide-react';
...
export default function CargaNotasCuantitativas({ cedula }: { cedula: string }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 2. Cargar Evaluaciones (si hay materia seleccionada)
        if (materiaNombre) {
          const evalRes = await fetch(`${API}/api/docente/evaluaciones?grado_id=${gradoId}&seccion=${seccion}&materia=${materiaNombre}&momento=${momento}&tipo=cuantitativa`, { headers: authHeaders() });
          const evalData = await evalRes.json();
          setEvaluaciones(evalData);

          // 3. Cargar Notas (si hay evaluaciones)
  ...
  const agregarEvaluacion = async (data: any) => {
    try {
      const res = await fetch(`${API}/api/docente/evaluaciones`, { method: 'POST', headers: authHeaders(), body: JSON.stringify({ ...data, grado_id: GRADOS_IDS[grado], seccion, materia: materiaNombre, momento, tipo: 'cuantitativa', docente_cedula: cedula }) });
      const nueva = await res.json();
      setEvaluaciones(prev => [...prev, nueva]);
      setShowModal(false);
      // Forzar recarga de notas para que el mapa se actualice con la nueva eval
      setNotas(prev => {
        const nuevoMap = { ...prev };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 max-w-[1600px] mx-auto pb-24">
      <div className="bg-white rounded-[2rem] px-6 md:px-10 py-6 md:py-8 mb-8 shadow-sm border-b-[8px] border-emerald-600 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h1 className="text-2xl md:text-4xl font-black text-slate-900 uppercase tracking-tighter italic leading-none">Notas Cuantitativas</h1>
          <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-[0.2em] mt-3">Sumatoria de evaluaciones planificadas</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowModal(true)} disabled={isClosed || !seccion} className="px-5 py-3 bg-orange-50 text-orange-600 rounded-xl font-black text-[10px] uppercase tracking-widest border-2 border-orange-200 hover:bg-orange-100 transition disabled:opacity-30"><Plus className="w-4 h-4 inline mr-1" /> Nueva Evaluación</button>
      ...
      <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-slate-200 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <div className="flex gap-2">{TODAS_SECCIONES.filter(s => seccionesDelGrado.includes(s)).map(s => (<button key={s} onClick={() => setSeccion(s)} className={`px-4 py-2 rounded-xl font-black text-sm ${seccion === s ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{s}</button>))}</div>
          </div>
      ...
      {/* TABLA: Se muestra SIEMPRE al elegir sección */}
      {!seccion ? (
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
              ...
              <thead className="bg-slate-50 border-b">
                <tr>
                  ...
                  {evaluaciones.length === 0 ? (
                    <th className="px-4 py-4 text-center text-[10px] font-black text-orange-400 uppercase italic">Sin evaluaciones planificadas</th>
                  ) : (
                    <>
                      {evaluaciones.map(ev => (
                        <th key={ev.id} className="px-2 py-4 text-center min-w-[100px]">
                          ...
                          <p className="text-[8px] text-orange-500 font-black">Máx: {ev.puntaje_maximo} pts</p>
                        </th>
                      ))}
                      <th className="px-4 py-4 text-center text-[10px] font-black text-emerald-600 bg-emerald-50">DEFINITIVA</th>
                    </>
                  )}
                </tr>
              </thead>
                          ...
                          {evaluaciones.map(ev => {
                            ...
                            return (
                              <td key={ev.id} className="px-2 py-2 text-center">
                                <input type="number" min="0" max={ev.puntaje_maximo} value={nota === null ? '' : nota} onChange={(e) => handleNotaChange(est.id, ev.id!, e.target.value, ev.puntaje_maximo)} disabled={isClosed} className="w-16 h-10 text-center font-black rounded-xl border-2 border-slate-200 focus:border-emerald-500 outline-none disabled:bg-slate-50" />
                              </td>
                            );
                          })}
          ...
          <div className="px-6 py-3 bg-slate-50 text-right text-[9px] text-slate-400">La Definitiva es la sumatoria de las evaluaciones cargadas.</div>
        </div>
      )}
...
function ModalEval({ open, onClose, onSave }: any) {
  ...
  if (!open) return null;
  const handleSubmit = () => { if (!nombre.trim()) return; onSave({ nombre, puntaje_maximo: puntaje }); onClose(); setNombre(''); setPuntaje(20); };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-[2rem] max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black">Nueva Evaluación</h3><button onClick={onClose}><X className="w-5 h-5 text-slate-400" /></button></div>
        <div className="space-y-4">
          <input type="text" placeholder="Ej: Prueba Escrita, Trabajo en Grupo..." value={nombre} onChange={e => setNombre(e.target.value)} className="w-full p-3 rounded-xl border-2 border-slate-200 font-black text-sm" />
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Puntaje Máximo de la Evaluación</p>
            ...
            <p className="text-[8px] text-orange-500 mt-1 font-bold">Ej: 20 pts, 15 pts, 5 pts (según criterio del docente)</p>
          </div>
        </div>
        <button onClick={handleSubmit} className="w-full mt-6 bg-slate-900 text-white py-3 rounded-xl font-black text-sm">Crear Evaluación</button>
      </div>
    </div>
  );
}
