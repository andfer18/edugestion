  ...
  const actionsPrincipales = [
    {
      ...
      items: [
        ...
        { label: 'Reporte (70% y 30%)', path: '#' },
        { label: 'Imprimir en Lote', path: '#' },
        { label: 'Calcular Promedios', path: '#' },
        { label: 'Actualizar Boletines', path: '#' },
      ]
    },
    {
      ...
      icon: ChartLine,
      items: [
        { label: 'Inscripciones Rend.', path: ROUTE_PATHS.PRE_INSCRIPCIONES },
        { label: 'Profesores Asignados', path: '#' },
        { label: 'Planilla Oficial (31059)', path: ROUTE_PATHS.REPORTES_MINISTERIO },
        { label: 'Reporte de Verificación', path: '#' },
        { label: 'Cierre de Rendimiento', path: '#' },
      ]
    },
    {
      ...
      icon: Scroll,
      items: [
        { label: 'Ver Notas Certificadas', path: '#' },
        { label: 'Importar desde Excel', path: '#' },
        { label: 'Constancias en Lote', path: '#' },
    ...
    },
    {
      ...
      icon: ClipboardList,
      items: [
        { label: 'Evaluación Continua', path: '#' },
        { label: 'Observaciones Boletín', path: '#' },
        { label: 'Estadísticas Generales', path: ROUTE_PATHS.CUADRO_HONOR },
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="text-right hidden lg:flex flex-col items-end gap-4">
              <div>
                ...
                <p className="text-5xl font-black italic tracking-tighter text-blue-600 leading-none">2025 — 2026</p>
              </div>
              <button 
                onClick={onLogout}
          ...
          <div className="lg:col-span-3 space-y-10">
                  ...
                  >
                    ...
                    <ul className="space-y-4">
                      {cat.items.map((item, j) => (
                        <li key={j}>
                          <a href={item.path} className="flex items-center gap-4 text-[11px] font-black uppercase text-slate-500 hover:text-blue-600 transition-colors group/item">
            ...
            <section className="bg-white rounded-[3.5rem] p-12 shadow-sm border-2 border-slate-100">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-6">
                {['1A','1B','2A','2B','3A','3B','4A','4B','5A','5B'].map(s => (
          ...
          <aside className="space-y-10">
            ...
            <section className="bg-white rounded-[3rem] p-10 shadow-sm border-2 border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <button className="bg-indigo-50 text-indigo-600 p-5 rounded-[2rem] flex flex-col items-center gap-3 transition-all duration-300 hover:bg-indigo-600 hover:text-white hover:scale-105">
                  <FileOutput className="w-6 h-6" />
                  <span className="text-[9px] font-black uppercase tracking-tight">Exportar</span>
                </button>
                <button className="bg-emerald-50 text-emerald-600 p-5 rounded-[2rem] flex flex-col items-center gap-3 transition-all duration-300 hover:bg-emerald-600 hover:text-white hover:scale-105">
                  <RefreshCw className="w-6 h-6" />
                  <span className="text-[9px] font-black uppercase tracking-tight">Actualizar</span>
                </button>
                <button onClick={() => navigate(ROUTE_PATHS.REPORTES_MINISTERIO)} className="bg-purple-50 text-purple-600 p-5 rounded-[2rem] flex flex-col items-center gap-3 transition-all duration-300 hover:bg-purple-600 hover:text-white hover:scale-105">
                  <PieChart className="w-6 h-6" />
                  <span className="text-[9px] font-black uppercase tracking-tight">Reportes</span>
                </button>
                <button className="bg-rose-50 text-rose-600 p-5 rounded-[2rem] flex flex-col items-center gap-3 transition-all duration-300 hover:bg-rose-600 hover:text-white hover:scale-105">
            ...
            <div className="grid grid-cols-1 gap-4">
              <a href={ROUTE_PATHS.HISTORICO_ESTUDIANTES} className="bg-white p-5 rounded-[2rem] border-2 border-slate-100 hover:border-blue-600 transition-all flex items-center justify-center gap-4 group">
                <History className="w-5 h-5 text-slate-400 group-hover:text-blue-600 transition-colors" />
                <span className="text-[11px] font-black uppercase tracking-widest text-slate-900 italic">Históricos Estudiantes</span>
              </a>
              <a href={ROUTE_PATHS.REPORTES_MINISTERIO} className="bg-white p-5 rounded-[2rem] border-2 border-slate-100 hover:border-blue-600 transition-all flex items-center justify-center gap-4 group">
                <Search className="w-5 h-5 text-slate-400 group-hover:text-blue-600 transition-colors" />
                <span className="text-[11px] font-black uppercase tracking-widest text-slate-900 italic">Reportes Oficiales MPPE</span>
              </a>
            </div>
